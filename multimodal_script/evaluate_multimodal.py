...
def evaluate_features(model, test_data, criterion, device, fold, args, mode="val", save_path=None, best_f1=None):
    model.eval()
    total_loss = 0.0
    y_true_patient = {}
    y_score_patient = defaultdict(list)
    ...
    with torch.no_grad():
        img_data, tab_data, patient_ids = test_data[0].to(device), test_data[1].to(device), test_data[2]
        targets = test_data[3].to(device)
